...
const AdvancedForm = ({ submitting, dispatch, backtestResult, styList, pagination }) => {
  ...
  const [index,
    setIndex] = useState(0);
  const [strategies,
    setStrategies] = useState([]);
  const [chartConfig,
    setChartConfig] = useState(initChartConfig(strategies));
  const [isEdit,
  ...
  const editStrategy = (strategy) => {
    ...
    newStrategy.factorList = newStrategy
      .factorList
      .map(item => ({
        ...item
      }));
  ...
  const onAddStrategy = (values, isOld) => {
    setError([]);
    const fundInfo = typeof values.fund === 'object'
      ? values.fund
      : fundInfoMap.get(values.fund);
  ...
  const saveSty = async (styInfo) => {
    const rs = await dispatch({
      type: styInfo._id
        ? 'fundValBackTest/updateSty'
        : 'fundValBackTest/addSty',
      payload: styInfo
    })
    console.log(rs)
    onSearchStyList();
    ...
    return [
      {
        ...
        render: (text, record) => {
          ...
          return (
            <div>
              <p>{fundName}({record.baseMoney}元)</p>
              <p>
                {fundCode}/{indexCode}
              </p>
            </div>
  ...
  return (
    <PageHeaderWrapper title="基金回测">
            ...
            <Col xl={6} lg={8} md={12} sm={24}>
              <Form.Item
                ...
                <Select onChange={onStyFundChange} placeholder="请选择基金">
                  {fundList.map(fund => (
                    <Option key={fund.fundCode} value={fund.fundCode}>{fund.fundName}</Option>
                  ))}
                </Select>
              </Form.Item>
            </Col>
        ...
        <TabPane key="2" tab="收藏列表">
          <Form
            ...
            <Row>
              ...
              <Col xl={6} lg={8} md={12} sm={24}>
                <Form.Item label="策略基金" name="fundCode" rules={[]}>
                  <Select placeholder="请选择策略基金">
                    <Option key="0001" value="">未选择</Option>
                    {fundList.map(fund => (
                      <Option key={fund.fundCode} value={fund.fundCode}>{fund.fundName}</Option>
                    ))}
                  </Select>
              ...
              <Col xl={6} lg={8} md={12} sm={24}>
                <Button type="primary" onClick={() => onSearchStyList()}>查询</Button>
              </Col>
            </Row>
          </Form>
          <Table
      ...
      <Card title="回测结果图表" className={styles.card} bordered={false}>
        <div id="chart1" className={styles.chartWrapper}></div>
      </Card>

      <FooterToolbar>
        {getErrorInfo(error)}
        <Button type="primary" onClick={() => styForm
          ?.submit()}>
          {isEdit
            ? '保存'
            : '添加'}策略
...
export default connect(({ fundValBackTest, loading }) => ({ backtestResult: fundValBackTest.backtestResult, styList: fundValBackTest.styList, pagination: fundValBackTest.pagination, submitting: loading.effects['fundValBackTest/fetchBacktestData'] }))(AdvancedForm);
